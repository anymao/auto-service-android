...
class AutoServiceRegisterPlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
        ...
        project.afterEvaluate {
            ...
            final android = project.getExtensions().getByType(AppExtension)
            android.applicationVariants.forEach { variant ->
                //产物目录
                final workDir = project.file("${project.buildDir}${File.separator}intermediates${File.separator}autoService${File.separator}${variant.dirName}${File.separator}")
                ...
                final compileTask = project.tasks.create("compileGenerated${variant.name.capitalize()}", JavaCompile.class){
